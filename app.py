...
def update_flashcard_size(size):
    st.session_state.flashcard_size = size
        ...
        if len(text.strip()) == 0:
            ...
            if 'flashcard_pairs' in st.session_state and st.session_state.flashcard_pairs:
                ...
                with st.expander("Customize Flashcards"):
                    ...
                    with col2:
                        # Use on_change callback to update size without triggering a full rerun
                        st.selectbox(
                            "Card Size:", 
                            ["small", "medium", "large"],
                            index=["small", "medium", "large"].index(st.session_state.flashcard_size),
                            key="size_selector",
                            on_change=update_flashcard_size,
                            args=("size_selector",)
                        )
